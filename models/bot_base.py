...
class BotBase(BaseModel):
    ...
    fiat_order_size: Amount = Field(default=0, ge=0, description="Min Binance 0.0001 BNB approx 15USD")
    ...
    cooldown: int = Field(default=0, ge=0, description="cooldown period in minutes before opening next bot with same pair")
    ...
    name: str = Field(default="terminal", description="Algorithm name or 'terminal' if executed from React app")
    status: Status = Field(default=Status.inactive)
    stop_loss: Amount = Field(default=0, ge=-1, le=101, description="If stop_loss > 0, allow for reversal")
    margin_short_reversal: bool = Field(default=False, description="Autoswitch from long to short or short to long strategy")
    take_profit: Amount = Field(default=0, ge=-1, le=101)
    trailling: bool = Field(default=False)
    trailling_deviation: Amount = Field(default=0, ge=-1, le=101, description="Trailling activation (first take profit hit)")
